...
class Space_Regulizer:
    ...
    def get_image_from_ws(self, image, w_codes, G):
        return torch.cat([G.synthesis(real_image, mask, w_code, noise_mode='none') for w_code in w_codes])

    def ball_holder_loss_lazy(self, image, new_G, num_of_sampled_latents, w_batch, use_wandb=False):
        loss = 0.0
        ...
        print("w samples shape", w_samples.shape)
        print("w batch shape", w_batch.shape)
        ...
        for w_code in territory_indicator_ws:
            ...
            with torch.no_grad():
                old_img = self.original_G.synthesis(image, mask, w_code)
    ...
    def space_regulizer_loss(self, image, new_G, w_batch, use_wandb):
        ret_val = self.ball_holder_loss_lazy(image, new_G, hyperparameters.latent_ball_num_of_samples, w_batch, use_wandb)
        return ret_val
